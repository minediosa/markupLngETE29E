...
- **Ročník:** 1.
- **Termín:** 1. května – 4. května 2027 (symbolický odkaz na vstup Polska do EU – 1. května 2004).
...
- **Rok:** 2027.
- **Halové uspořádání:** Haly A–D (každá má 30 stánků).
...
- Výhody „going local".
- Startupy a inovace.
- Bezpečnost a ochrana soukromí (Linux, Proton, sledování ze strany velkých technologických firem typu Microsoft).
...
Inspirace webu: viz styl shirtz.cool (před odevzdáním odstranit všechny brandové odkazy a zmínky).

## 6. Technická specifikace

### 6.1 Aktualizovaný `validator.xsd`

- Detekuje chybějící či špatně pojmenované elementy (všechny povinné mají `use="required"`).
- Přidává povinná pole pro každého vystavovatele: `website`, `representative` a `contactEmail`.
- Kategorie jsou výčtem (`Security`, `Technology`, `Software`, `Mix`, `Startup`) – lze snadno rozšířit.
- Definuje globální typy: `time`, `hallNumber`, `email`, `url`.

**Omezení:**

- Min. 60, max. 80 vystavovatelů (rezerva pro startupy).
- Min. 3, max. 5 dní.
- Min. 2 místa (scény) na den.
- Časové úseky `HH:00` nebo `HH:30`.

### 6.2 Program

- Tři scény mohou běžet paralelně (přednášky, workshopy, herní aréna).
- **Přednášky a workshopy:** 1 hodina každá, s povinnou přestávkou na oběd 14:00–15:00 (žádná aktivita).
...
- **Denní časový rámec:** 10:00–22:00. Přednášky a workshopy končí nejpozději v 19:00, následuje volná hodina pro neformální networking.

**Ukázka jednoho dne (XML fragment):**

```xml
<day id="1" date="2027-05-01">
    <!-- Talks (large stage) -->
    <event id="t1" stageRef="talks">
        <title>Opening Keynote – European Digital Sovereignty</title>
        <presenter>European Commission</presenter>
        <time>10:00</time>
    </event>
    <!-- Obědová pauza 14:00–15:00 (žádné přednášky ani workshopy) -->
    <!-- Gaming (small stage) -->
    <event id="g1" stageRef="games">
        <title>Gaming Showcase – EU Indie Strategy (10:00–14:00)</title>
        <presenter>GameDev EU</presenter>
        <time>10:00</time>
    </event>
</day>
```

### 6.3 Definice vystavovatele (exhibitor)
...
| `<contactEmail>` | E-mail pro kontakt. |
| atribut `id` (povinný) | Jedinečný celočíselný identifikátor. |
...
- Dokumentace (`README.md`, `specification.md`) je v čistém Markdownu.Přidává povinné pole pro každého vystavovatele: website, representative a contactEmail.
